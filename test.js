...
test('should only report on given filter path', t => {
  let state = didt({ a: { b: 1, c: 3 } }, { filter: 'a.b', print: false });
  state.a.c = 42;
  state.a.b = 42;
  t.is(stats(state).length, 1);
  firstPath(t, state, 'a.b');
...
test('should not print if print false', t => {
  let w = console.warn;
  console.warn = () => t.fail();
  let state = didt({ a: 1 }, { print: false });
  state.a = 42;
  console.warn = w;
  t.pass();
});
